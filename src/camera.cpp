...
void Camera::updateViewMatrix() {
   ...
   */
  front = rotation * original_front; // front after rotation
  up = rotation * original_up; // up after rotation
  right = glm::cross(front, original_up); // right is the cross product of front and up
  viewMatrix = glm::lookAt(position, position+front, up); // use lookAt to calculate viewMatrix
}
...
void Camera::updateProjectionMatrix(float aspectRatio) {
   ...
   */
  projectionMatrix = glm::perspective(FOV, aspectRatio, zNear, zFar); // use perspective() function to calculate the projectionMatrix
}